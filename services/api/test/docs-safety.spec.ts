 ...
 * must exist, and the Phase 17 calibration and Phase 18 dataset docs
 * must carry no stream scheme, URL, drive-letter path, or
 * credential-slot-looking string. (The Phase 16 doc deliberately NAMES
 * the rtsp scheme and the env-slot pattern when explaining
 * operator-managed configuration, so only the existence check applies
 * to it.)
 *
...
describe('CV docs safety', () => {
  ...
  const phase18Doc = join(docsDir, 'dataset-improvement-model-tuning.md');
  const strictDocs = [phase17Doc, phase18Doc];

  it('the Phase 16, 17, and 18 operator docs exist', () => {
    expect(existsSync(phase16Doc)).toBe(true);
    expect(existsSync(phase17Doc)).toBe(true);
    expect(existsSync(phase18Doc)).toBe(true);
  });

  it.each(strictDocs)(
    '%s contains no stream scheme, URL, drive path, or credential slot text',
    (doc) => {
  ...
  it('the dataset doc never promises model improvement percentages', () => {
    const text = readFileSync(phase18Doc, 'utf8');
    expect(text).not.toMatch(/\d+(\.\d+)?\s*%\s*(more\s+)?(accura|improve)/i);
  });

  it('all docs restate the shadow-only safety contract', () => {
    for (const doc of [phase16Doc, phase17Doc, phase18Doc]) {
