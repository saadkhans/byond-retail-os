...
- **Coverage gates follow the run's purpose.** Only the class families
  ...
  calibration-validation runs gate on both families. False-touch
  filtering is a **binary** task, so its gates check exactly two
  classes — NO_OP negatives and positive touches (a PICKUP or a RETURN
  both count as positive coverage) — rather than every individual
  action label. The stable assignment itself is identical for every
  purpose.
...
then export again. Calibration **content** is checked too: candidate
refresh captures a fingerprint of safe calibration metadata (profile
version, orientation, mount, zone counts, and zone membership — never
zone labels, polygons, or product details), and export recomputes and
compares it. Editing the profile or **adding, updating, or deleting**
any of its zones after the last candidate refresh therefore rejects the
export — including a re-export of an already `EXPORTED` run, which can
never silently return a different calibration snapshot. The manifest's
calibration section is built from the very same read the fingerprint
validated, so there is no window in which a concurrent calibration edit
could pass validation yet change the emitted metadata. FILE_REPLAY-only
runs are never stamped, so calibration edits never block them. Once `EXPORTED`, the run is frozen: candidate
